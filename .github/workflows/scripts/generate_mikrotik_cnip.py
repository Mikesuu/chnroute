...
COMMENT = "China IP - Generated by Mikesuu/adrules"
...
def generate_mikrotik_script(cidrs):
    ...
    script_lines.append(f"#: Source: {CIDRS_CN_URL}")
    script_lines.append(f"#: List Name: {ADDRESS_LIST_NAME}")
    script_lines.append(f"#: Count: {len(cidrs)}")
    script_lines.append("")
    
    # 1. 移除旧列表 (可选，但推荐，以防CIDR变动导致列表残留)
...
def main():
    ...
    print(f"[DONE] Mikrotik script saved to {OUTPUT_FILE} ({len(cidrs)} entries)")
